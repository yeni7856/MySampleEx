...
namespace MySampleEx
{
    public class MeeleWeapon : MonoBehaviour
    {
        ...
        public AttackPoint[] attackPoints = new AttackPoint[0];         //공격포인트
...
#if UNITY_EDITOR
                attackPoints[i].previousPosition.Clear();
                attackPoints[i].previousPosition.Add(m_PreviousPos[i]);
#endif
            }
            m_InAttack = true;
        }
        ...
        private void FixedUpdate()
        {
            if(m_InAttack)
            {
                ...
                for(int i = 0; i < attackPoints.Length;i++)
                {
                    ...
                    int contacts = Physics.SphereCastNonAlloc(r, apt.radius,
        ...
        private void CheckDamage(Collider other, AttackPoint apt)
        {
            ...
            data.dmgSource = m_Owner.transform.position;
